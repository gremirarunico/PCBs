...
def PC_CONFIGURE():
    ...
    print(respons)
    time.sleep(0.1)
    powerConverter.write(bytes("set dt " + str(dt) + "\r\n", encoding='ascii'))
    respons = powerConverter.readline()
    print(respons)
    time.sleep(0.1)
    powerConverter.write(bytes("set adt " + str(adt) + "\r\n", encoding='ascii'))
    respons = powerConverter.readline()
    print(respons)
    time.sleep(0.1)
...
powerConverter = serial.Serial(port="COM", baudrate=115200, bytesize=8, timeout=2, stopbits=serial.STOPBITS_ONE)
...
dataStored = [["DT", "ADT", "Frequency", "Power1", "Power2", "Power3", "Current1",  "Current2",  "Current3", "Voltage1", "Voltage2", "Voltage3", "VoutRMS", "IoutRMS"]]

start = 700000
stop = 1500000
step = 50000

#for freq in [1000000, 1250000, 1500000]:
for dt in range(20, 61, 1):
    for adt in range(0, 61, 1):
        for freq in range(start, stop+1, step):
            ...
            ioutrms = multimeter.query(":MEASure:CURRent?")[0:-1]
            ioutrms = multimeter.query(":MEASure:CURRent?")[0:-1]
            print("Current: " + str(ioutrms))
            
            dataStored.append([str(dt), str(adt), str(freq), powers[0], powers[1], powers[2], currents[0], currents[1], currents[2], voltages[0], voltages[1], voltages[2], voutrms, ioutrms])
